...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool bDrawIndicator, bool bStatus )
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if ( bDrawIndicator )
    {
        ...
        it_anim->second.fOpacity = ImLerp( it_anim->second.fOpacity, bStatus ? 1.f : 0.f, g.IO.DeltaTime * 10.f );
        it_anim->second.vColor = ImLerp( it_anim->second.vColor, bStatus ? ImVec4{ 66.f / 255.f, 141.f / 255.f, 245.f / 255.f, 1.f } : ImVec4{ 1.f, 1.f, 1.f, 1.f }, g.IO.DeltaTime * 10.f );
        auto fOpacity = it_anim->second.fOpacity;
        window->DrawList->AddRectFilledMultiColor( { bb.Min.x, bb.Max.y - 4 }, { bb.Max.x, bb.Max.y }, ImColor( 15, 15, 15, (int)( fOpacity * 255.f ) ), ImColor( 15, 15, 15, ( int )( fOpacity * 255.f ) ), ImColor( 66, 141, 245, ( int )( fOpacity * 255.f ) ), ImColor( 66, 141, 245, ( int )( fOpacity * 255.f ) ) );
    }
    ...
    if ( bDrawIndicator )
    {
        PushStyleColor( ImGuiCol_Text, it_anim->second.vColor );
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, bool bDrawIndicator, bool bStatus )
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, bDrawIndicator, bStatus );
}
...
bool ImGui::Checkbox( const char* label, bool* v )
{
    ...
    it_anim->second.opacity = ImLerp( it_anim->second.opacity, *v ? 1.f : 0.f, g.IO.DeltaTime * 10.f );

    window->DrawList->AddRectFilled( ImVec2( check_bb.Min.x, check_bb.Min.y ), ImVec2( check_bb.Max.x, check_bb.Max.y ), ImColor(32, 32, 32 ), 4.f );
    RenderCheckMark( window->DrawList, check_bb.Min + ImVec2( pad + 2.0f, pad + 2.0f ), ImColor( 230, 230, 255, ( int )( it_anim->second.opacity * 255.f ) ), 14.f );
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags )
{
    ...
    const float size = GetWindowWidth( ) - 60;

    const ImRect rect( window->DC.CursorPos, window->DC.CursorPos + ImVec2( size, 23 ) );
    const ImRect clickable( window->DC.CursorPos + ImVec2( 0, 0 ), window->DC.CursorPos + ImVec2( size, 23 ) );

    const ImVec2 arrow_size = ImGui::CalcTextSize( "-" );
    ...
    it_anim->second.open_anim = ImLerp( it_anim->second.open_anim, popup_open ? 1.0f : 0.0f, g.IO.DeltaTime * 10.f );
    it_anim->second.arrow_anim = ImLerp( it_anim->second.arrow_anim, popup_open ? 1.f : 0.0f, g.IO.DeltaTime * 10.f );
    ...
    RenderTextClipped( clickable.Min + ImVec2( 14, 1 ), clickable.Max - ImVec2( 24, -47 ), preview_value, NULL, NULL, ImVec2( 0.0f, 0.0f ) );

    window->DrawList->AddText( ImVec2( clickable.Max.x - 20, ( clickable.Min.y + clickable.Max.y ) / 2 - arrow_size.y / 2 ), ImColor( 1.0f, 1.0f, 1.0f, 0.2f + it_anim->second.arrow_anim ), "-" );
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddLine( ImVec2( frame_bb.Min.x, frame_bb.Max.y - 5  ), ImVec2( frame_bb.Max.x, frame_bb.Max.y - 5 ), ImColor( 150, 150, 150 ), 2.0f );
    ...
    window->DrawList->AddLine( ImVec2( frame_bb.Min.x, frame_bb.Max.y - 5 ), ImVec2( grab_bb.Max.x, frame_bb.Max.y - 5 ), ImColor( 230, 230, 255 ), 2.0f );

    if ( grab_bb.Max.x > grab_bb.Min.x )
        window->DrawList->AddCircleFilled( grab_bb.Max - ImVec2( 5, 2 ), 6, ImColor(230, 230, 255 ), 100.f );
    ...
    auto m_TextSize = ImGui::CalcTextSize( value_buf );
    window->DrawList->AddText( ImVec2( frame_bb.Max.x - m_TextSize.x, frame_bb.Min.y - 6 ), ImColor( 255, 255, 255 ), value_buf );

    if ( label_size.x > 0.0f )
        RenderText( ImVec2( frame_bb.Min.x, frame_bb.Min.y - 6 ), label );
...
bool ImGui::ColorEdit4 ( const char* label, float col[ 4 ], ImGuiColorEditFlags flags )
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const float square_sz = GetFrameHeight ( ) + 10;
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg )
{
    ...
    else
    {
        ...
        window->DrawList->AddRectFilled( ImVec2( bb_inner.Min.x, bb_inner.Min.y ), ImVec2( bb_inner.Max.x, bb_inner.Max.y ), GetColorU32( col_source ), 4.f );
